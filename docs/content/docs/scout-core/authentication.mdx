...
sequenceDiagram
    ...
    participant Edge as Edge Server (Middleware)
    ...
    Note over User,Edge: Session Validation
    Client->>Edge: Request with JWT in cookies
    Note over Edge: Validate JWT locally<br/>No database call needed
    alt JWT valid
        Edge->>Client: Allow request
